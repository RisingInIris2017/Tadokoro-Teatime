...
第一阶段，Sakura、潘马斯农场的果树和热量与气候的果树对玩家开放。
...
目前主要的模组调整是**移除根源魔法、玩家负重、格鲁怪物 MOD，新增斯巴达武器和地底矿界**。
...
将豆腐工艺调整到 maid 游戏阶段。
##### 新增深渊国度模组
深渊国度模组负责 dcs_tech 阶段黑暗之城（失落之城+失落之魂+深渊国度）领域的生成，10th 正作最终维度——深渊荒芜之地的生成，

添加一批新的游戏内容，以及三个 DLC 世界。
##### 新增玩家负重模组
玩家背包中的物品过多会受到缓慢 Buff、饥饿 Buff 的惩罚。

对潜影盒、热量与气候钢箱子、工具钢剑、HAC 十字弩的惩罚尤其突出。
#### 2020.3.19 更新
##### 新增 Grue 模组
Grue 模组增加了一种在黑暗处生成的生物，*无敌*，且*会攻击玩家*，但一到亮度高的地方就会*立即消失*。

设定是亮度低于 4(0, 1, 2, 3) 时生成，亮度达到 4 时消失。

仅生成于*除了下界和末地以外的世界*。

~~这种生物不会干扰前期玩家的游戏进程，在 furnace 阶段加入游戏。~~

目前版本 Grue 一开始就会加入游戏，还在与模组作者协调当中。
#### 2020.3.24 更新
##### 调整豆腐工艺的阶段
现在仅豆腐世界及相关内容（包括钻石豆腐、豆腐魔杖）放到 `maid` 阶段，其余内容移到 `pams` 阶段。
##### 调整盖亚魔典三级怪的游戏阶段
将盖亚魔典的四种三级怪（吸血鬼、牛魔王、斯芬克斯、女武神）移动到 `pams` 阶段。
##### 调整盖亚魔典饰品的游戏阶段
将盖亚魔典的所有饰品，以及四种无效的指环移动到 `pams` 游戏阶段。
##### 调整技能书模组的游戏阶段
将技能书模组移动到 `furnace` 游戏阶段。
_______________
## 2. 游戏背景故事设定
<pre>
